	...
	public IntPair[] findIntersections(IntRectangle[] rectSet1, IntRectangle[] rectSet2) {
		...
		for (IntRectangle r : rectSet1) {
			...
			for (IntRectangle s : rectSet2) {
				...
				if (left1 >= left2 && left1 <= right2) xOverlap = true;
				if (right1 >= left2 && left1 <= right2) xOverlap = true; 
				if (top1 >= bottom2 && top1 <= top2) yOverlap = true;
				if (bottom1 >= bottom2 && bottom1 <= top2) yOverlap = true;
